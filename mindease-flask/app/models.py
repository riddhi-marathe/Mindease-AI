...
class User(db.Model):
    ...
    assessments = db.relationship('HealthAssessment', backref='user', lazy=True, cascade='all, delete-orphan')
...
class HealthAssessment(db.Model):
    ...
    assessment_type = db.Column(db.String(50), nullable=False)  # 'symptom_check', 'wellness', 'mental_health'
...
class HealthMetric(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    metric_type = db.Column(db.String(50), nullable=False)  # 'heart_rate', 'blood_pressure', 'weight', etc
